...
import { formatDuration, formatTime, getSummary, incidentMetrics, sortForOperations } from './utils';

function NpdBanner({ incidents }) {
  const count = incidents.filter((item) => item.type === 'NPD').length;
  return count ? <div className="global-npd-banner"><div className="alert-icon">!</div><div><strong>NPD ACTIVE — PRODUCTION STOPPED</strong><span>{count} NPD incident{count > 1 ? 's' : ''} active. Normal reporting remains blocked until all are resolved.</span></div></div> : null;
}
...
function ReportModal({ incidents, onClose, onSubmit }) {
  ...
  return <Modal title="Report Breakdown" onClose={onClose}>{npdActive && <div className="npd-warning"><div className="alert-icon">!</div><div><strong>NPD is already active</strong><span>Only another NPD breakdown can be reported until all NPD incidents are resolved.</span></div></div>}<form className="stack-form" onSubmit={submit}>
...
function IncidentTable({ incidents, clock, onSelect }) {
  return <div className="table-wrap"><table className="incident-table"><thead><tr><th>Ticket</th><th>Incident</th><th>Owner</th><th>Status</th><th>Priority</th><th>Incident SLA</th><th>Attending</th></tr></thead><tbody>{incidents.map((item) => { const metrics = incidentMetrics(item, clock); return <tr key={item.id} onClick={() => onSelect(item)}><td><strong>{item.id}</strong>{item.type === 'NPD' && <span className="table-npd">NPD</span>}</td><td><strong>{item.problem}</strong><small>{item.line} · {item.belt}</small></td><td>{item.department}</td><td><StatusPill status={item.status}/></td><td><span className={`priority-pill priority-${item.priority.toLowerCase()}`}>{item.priority}</span></td><td className={metrics.overdue ? 'overdue-text' : ''}>{formatDuration(metrics.incidentMs)}</td><td>{item.acknowledgedBy || '—'}</td></tr>; })}</tbody></table>{incidents.length === 0 && <div className="empty-table">No incidents match the selected filters.</div>}</div>;
}
...
export function ManagementDashboard({ session, incidents, clock, actions, onLogout }) {
  ...
  return <main className="app-shell"><Topbar title={hodDepartment ? `${hodDepartment} HOD` : 'Plant Admin'} subtitle={hodDepartment ? 'Department Operations' : 'Plant Operations'} session={session} onLogout={onLogout} actions={<button className="icon-label-button" onClick={() => location.assign('/tv')}>TV Display</button>}/><section className="admin-wrap"><NpdBanner incidents={scoped}/><SummaryCards summary={summary} department={hodDepartment}/>
    ...
    <div className="dashboard-grid"><section className="panel critical-panel"><div className="panel-heading"><div><p className="eyebrow">PRIORITY</p><h2>Critical Incidents</h2></div></div>{sortForOperations(scoped, clock).filter((item) => item.priority === 'Critical' || item.type === 'NPD').slice(0,5).map((item) => <button className="critical-row" key={item.id} onClick={() => setSelected(item)}><div><strong>{item.id} · {item.problem}</strong><small>{item.department} · {item.line}</small></div><span>{formatDuration(incidentMetrics(item, clock).incidentMs)}</span></button>)}</section>
...
export function TvDisplay({ incidents, clock }) {
  ...
  return <main className="tv-screen"><header className="tv-header"><div><p>YAZAKI · LIVE PLANT OPERATIONS</p><h1>Breakdown Monitoring</h1></div><div className="tv-clock">{new Date(clock).toLocaleTimeString()}</div></header><NpdBanner incidents={incidents}/><SummaryCards summary={summary}/><div className="tv-table"><div className="tv-row tv-table-head"><span>Ticket / Priority</span><span>Problem</span><span>Location</span><span>Status</span><span>Incident Duration</span><span>ACK</span><span>Attending</span></div>{sorted.slice(page * pageSize, page * pageSize + pageSize).map((item) => { const metrics = incidentMetrics(item, clock); return <div className={`tv-row ${item.type === 'NPD' ? 'tv-npd' : ''}`} key={item.id}><span><strong>{item.id}</strong><small>{item.type === 'NPD' ? 'NPD · ' : ''}{item.priority}</small></span><span><strong>{item.problem}</strong><small>{item.department}</small></span><span>{item.line}<small>{item.belt}</small></span><span><StatusPill status={item.status}/></span><span className={metrics.resolutionOverdue ? 'overdue-text' : ''}>{formatDuration(metrics.incidentMs)}</span><span className={metrics.ackOverdue ? 'overdue-text' : ''}>{item.status === 'WAITING_ACK' ? `${metrics.ackOverdue ? '+' : ''}${formatDuration(Math.abs(metrics.ackMs))}` : 'ACK'}</span><span>{item.acknowledgedBy || '—'}</span></div>; })}</div><footer className="tv-footer"><span>Read only · Simulated frontend data</span><span>Page {page + 1} of {pages} · Auto changes every 10 seconds</span></footer></main>;
}
