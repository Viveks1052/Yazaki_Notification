...
export function IncidentCard({ incident, clock, onAcknowledge, onResolve, onDetails, onReassign, locked }) {
  const metrics = incidentMetrics(incident, clock);
  return <article className={`issue-card ${incident.type === 'NPD' ? 'npd-card' : ''} ${locked ? 'faded-card locked-incident' : ''}`} aria-disabled={locked || undefined}>
    ...
    <div className="issue-footer"><div className="timeline-copy"><span>Reported {formatTime(incident.createdAt)}</span>{incident.acknowledgedBy && <span className="attendee-copy">• {incident.acknowledgedBy}</span>}</div>{locked ? <div className="npd-lock-notice"><span aria-hidden="true">🔒</span><div><strong>Locked while NPD is active on this conveyor belt.</strong><small>Timer and original incident state continue unchanged</small></div></div> : <div className="card-actions"><button className="ghost-small" onClick={() => onDetails(incident)}>DETAILS</button>{incident.status === 'WAITING_ACK' ? <>{incident.type === 'NPD' && <button className="ghost-small" onClick={() => onReassign(incident)}>REASSIGN</button>}<button className="secondary-action" onClick={() => onAcknowledge(incident)}>ACKNOWLEDGE</button>{incident.type === 'NPD' && <button className="resolve-action" onClick={() => onResolve(incident)}>RESOLVE</button>}</> : <><button className="ghost-small" onClick={() => onReassign(incident)}>REASSIGN</button><button className="resolve-action" onClick={() => onResolve(incident)}>RESOLVE</button></>}</div>}</div>
  </article>;
}
...
export function IncidentDrawer({ incident, clock, onClose, onReassign }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="incident-drawer"><div className="drawer-header"><div><p className="eyebrow">INCIDENT DETAILS</p><h2>{incident.id}</h2></div><button className="icon-button" onClick={onClose}>×</button></div><div className="drawer-body">
    ...
    <section className="detail-section"><div className="detail-heading"><h3>Current owner</h3>{(incident.status === 'ACTIVE' || incident.type === 'NPD') && <button className="text-button" onClick={() => onReassign(incident)}>Reassign department</button>}</div><p>{incident.department} Department</p><small>{incident.acknowledgedBy || 'Awaiting attending employee'}</small></section>
